...
def eom(t, Z):
    ...
    B[0, 0] = K * (sin_phi * sin_psi + cos_phi * cos_psi * sin_theta) * omega_sum - Ax * vx + wind_force_x
    B[1, 0] = -Ay * vy - K * (cos_psi * sin_phi - cos_phi * sin_psi * sin_theta) * omega_sum + wind_force_y
...
plt.title('Trajetória do Quadricóptero com Força de Vento')
plt.legend()
plt.grid(True)
plt.show()
